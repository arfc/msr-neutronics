...
xsecs=(B1_FLX B1_REMXS B1_FISS B1_NUBAR B1_NSF B1_KAPPA B1_DIFFCOEF B1_INVV)
start_cols=()
stop_cols=()
...
for temp in ${temp_range[@]}; do
    ...
    for index in "${!xsecs[@]}"; do
        awk -v start_column=${start_cols[index]} -v stop_column=${stop_cols[index]} -v temp=$temp -v xsec=${xsecs[index]} \
            'BEGIN {ORS = ""} $1==xsec {j++} j==1 {print temp" "; for (i=start_column; i <= stop_column; i=i+2) print $i" "; print "\n"; exit}' $res_file >> "${interp_dir}/${root}_all_${xsecs[index]}.txt"
        awk -v start_column=${start_cols[index]} -v stop_column=${stop_cols[index]} -v temp=$temp -v xsec=${xsecs[index]} \
            'BEGIN {ORS = ""} $1==xsec {j++} j==2 {print temp" "; for (i=start_column; i <= stop_column; i=i+2) print $i" "; print "\n"; exit}' $res_file >> "${interp_dir}/${root}_fuel_${xsecs[index]}.txt"
        awk -v start_column=${start_cols[index]} -v stop_column=${stop_cols[index]} -v temp=$temp -v xsec=${xsecs[index]} \
            'BEGIN {ORS = ""} $1==xsec {j++} j==3 {print temp" "; for (i=start_column; i <= stop_column; i=i+2) print $i" "; print "\n"; exit}' $res_file >> "${interp_dir}/${root}_moder_${xsecs[index]}.txt"
    done
done

cd $interp_dir
for f in *B1*.txt; do mv "$f" "`echo $f | sed 's/_B1//'`"; done
for f in *.txt; do mv "$f" "`echo $f | sed 's/^/B1_/'`"; done
